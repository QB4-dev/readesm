...
EsmFile::EsmFile(const QString& filename) : fileData(loadFile(filename)), errors() {
	if (fileData.size() == 0) {
		errors = tr("Could not read any data from file '%1'.").arg(filename);
	}
	...
	if (ov) {
		...
		if (mc) {
			caCertificate = &mc->certificate;
		}
		QSharedPointer<CardCertificate> dc = findTypeInVector<TopLevelBlock, CardCertificate>(blocks);
		if (dc) {
			deviceCertificate = &dc->certificate;
		}
	}
	if (caCertificate) {
		caCertificate->attemptVerificationFrom(ercaKey);
	}
	if (deviceCertificate) {
		deviceCertificate->attemptVerificationFrom(*caCertificate);
	}
