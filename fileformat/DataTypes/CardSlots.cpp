...
void CardSlots::printOn(Reporter& report) const {
	...
	} else if (!cardNumberDriverSlotBegin.isDefaultValue()) {
		report.tagValuePair(tr("cardNumberDriverSlot"), cardNumberDriverSlotBegin.toString());
	}
	...
	} else if (!cardNumberCoDriverSlotBegin.isDefaultValue()) {
		report.tagValuePair(tr("cardNumberCoDriverSlot"), cardNumberCoDriverSlotBegin.toString());
	}
}
