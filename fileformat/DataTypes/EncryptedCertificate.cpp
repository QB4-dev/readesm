...
bool EncryptedCertificate::attemptVerification(const RsaPublicKey& key) {
	...
	if (srdash.size() != 128 || srdash.at(0) != 0x6A || (unsigned char)srdash[127] != 0xBC) {
		return false;
	}
	...
	if (!checkSha1(RawData(cdash), hdash)) {
		return false;
	}
...
bool EncryptedCertificate::checkSignature(const RawData& signedData, const RawData& signature) const {
	...
	QByteArray hdash = srdash.mid(107, 20);
	if (!checkSha1(signedData, hdash)) {
		return false;
	}
	const unsigned char der[] = { 0x30, 0x21, 0x30, 0x09, 0x06, 0x05, 0x2b,
			0x0e, 0x03, 0x02, 0x1a, 0x05, 0x00, 0x04, 0x14 };
	for (int j = 0; j < 15; ++j) {
		if ((unsigned char)srdash.at(92 + j) != der[j]) {
			return false;
		}
	}
	for (int j = 1; j < 91; ++j) {
		if ((unsigned char)srdash.at(j) != 0xff) {
			return false;
		}
	}
...
bool EncryptedCertificate::attemptVerificationFrom(const EncryptedCertificate& certificate) {
	return certificate.isVerified() && attemptVerification(certificate.decryptedCertificate->rsaPublicKey);
}
