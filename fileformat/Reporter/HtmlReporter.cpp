	...
	if (nestLevel == 1) {
		...
		*this << value;
		...
		if (!value.toString().isEmpty()) {
			if (tag.isEmpty()) {
				collector << toggleAbleBlocks(value.toString().replace('&', "&amp;"), false);
			} else {
				collector << toggleAbleBlocks(QString("%1: <b>%2</b>").arg(lTag, value.toString().replace('&', "&amp;")), false);
			}
			*this << value;
			collector << "</ul></li>";
		} else {
...
QString HtmlReporter::toggleAbleBlocks(const QString& title, bool showByDefault) {
	++toggleNumber;
	return QString("\n<li>%1 (<a href='#link%2' onclick='return toggle_visibility(%2);' name='link%2' id='link%2'>%3</a>)<ul style='%4' name='toggled%2' id='toggled%2'>")
			.arg(title)
			.arg(toggleNumber)
			.arg(showByDefault ? tr("hide") : tr("show"))
			.arg(showByDefault ? QString() : "display:none");
}
...
void HtmlReporter::arrayStart(int count, const QString& title, bool defaultShown) {
	if (count > 0) {
		...
		collector << "<li>" << tr("No %1.").arg(title) << "</li>";
	}
}
