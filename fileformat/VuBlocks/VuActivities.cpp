	...
	if (activityChangeInfos.numberOfBlocks() > 0) {
		for (int j = 0; j < activityChangeInfos.numberOfBlocks() - 1; ++j) {
			if (activityChangeInfos[j + 1].t != 0) {
				activityChangeInfos[j].setDuration(activityChangeInfos[j + 1].t - activityChangeInfos[j].t, true);
			} else {
				activityChangeInfos[j].setDuration(60*24 - activityChangeInfos[j].t, true);
			}
		}
...
void VuActivities::printOn(Reporter& report) const {
	...
	if (!iwCondensed.isEmpty()) {
		report.tagValuePair(tr("Drivers"),iwCondensed);
	}
	...
	if (report.allowSvg()) {
		for (int slot = 0; slot < 2; ++slot) {
			...
			if (count > 1) {
				report.tagValuePair(slot == 0 ? tr("Activities Driver") : tr("Activities Codriver"), visualization.toString());
			}
		}
	}
